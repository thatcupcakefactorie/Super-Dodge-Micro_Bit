function startGame (difficulty: number) {
    ...
    LoopCount = 0
    while (true) {
        basic.pause(2000 / ((LoopCount / 50 + 1) * (Difficulty + 1) / 3))
        if (!(GameOver)) {
            if (Enemy.length > 0) {
...
function gameOverSequence () {
    music.stopAllSounds()
    if (Enemy.length > 0) {
        for (let index = 0; index <= Enemy.length - 2; index++) {
            Enemy.pop().delete()
        }
    }
    Player.delete()
    ...
    music.play(music.createSoundExpression(WaveShape.Square, 408, 1, 98, 0, 2000, SoundExpressionEffect.None, InterpolationCurve.Linear), music.PlaybackMode.InBackground)
    basic.pause(2000)
    control.reset()
}
